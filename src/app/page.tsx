...
export default function Home() {
  ...
  const confirmUpdate = () => {
    localStorage.setItem("xalanify_v_track", "0.14.0");
    setShowPopup(false);
  };
      ...
      {showPopup && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
          <div className="bg-zinc-900 border border-white/10 rounded-[2.5rem] p-8 w-full max-w-xs text-center shadow-2xl">
            <h2 className="text-xl font-black mb-2 text-white">Versão 0.14.0 🔥</h2>
            <p className="text-zinc-400 text-xs mb-6">Áudio desbloqueado e créditos ao Xalana adicionados!</p>
            <button onClick={confirmUpdate} className="w-full py-4 bg-white text-black font-black rounded-2xl">OK!</button>
          </div>
        </div>
      )}
    </div>
