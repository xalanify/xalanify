...
import { Search as SearchIcon, Play, Loader2, CheckCircle, Music } from "lucide-react";
    ...
    <div className="p-6 pb-40 min-h-full">
      ...
      <form onSubmit={handleSearch} className="relative mb-8">
        <input 
          ...
          className="w-full bg-zinc-900 border border-white/5 p-5 pl-14 rounded-[2rem] outline-none focus:border-white/20 transition-all font-bold text-sm shadow-2xl"
        />
      ...
      <div className="space-y-3">
        {loading ? (
          <div className="flex flex-col items-center py-20 gap-4 opacity-20">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">A sintonizar...</p>
          </div>
        ) : searchResults.length > 0 ? (
          searchResults.map((track) => (
            <div 
              key={track.id} 
              className="flex items-center justify-between p-3 hover:bg-white/5 rounded-[2rem] transition-all group"
            >
              <div onClick={() => playTrack(track)} className="flex items-center gap-4 flex-1 cursor-pointer overflow-hidden">
                ...
                <div className="truncate">
                  <p className={`text-sm font-bold truncate ${currentTrack?.id === track.id ? 'text-purple-400' : ''}`} style={{ color: currentTrack?.id === track.id ? themeColor : '' }}>
                    {track.title}
                  </p>
          ...
          <div className="text-center py-20 opacity-20">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.3em]">Pesquisa algo novo</p>
          </div>
        )}
      </div>
    </div>
