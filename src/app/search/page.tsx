...
export default function SearchPage() {
  ...
  const handlePlay = async (track: any) => {
    ...
    addLog(`A obter stream para: ${track.title}`);
    ...
    try {
      ...
      if (ytId) {
        ...
        setIsPlaying(true);
        addLog("Sucesso: Áudio carregado");
      } else {
        addLog("Erro: Vídeo não encontrado");
      }
    } catch (err) {
      addLog("Erro na conversão para YouTube");
    } finally {
        ...
        <div className="space-y-2">
          {searchResults.map((track) => (
            <div key={track.id} className="flex items-center justify-between group">
              <div onClick={() => handlePlay(track)} className="flex-1 flex items-center gap-4 p-3 hover:bg-white/5 rounded-[2rem] transition-all cursor-pointer">
