...
async function tryRelinkLegacyContent(userId: string) {
  ...
  const baseUsername = normalizeUsernameCandidate(usernameFromEmail(authUser.email))
  if (!baseUsername) return

  const { error } = await supabase.rpc("relink_my_legacy_content", {
    target_email: authUser.email,
    target_username: baseUsername,
  })
...
export async function listShareTargets(currentUserId: string, isAdmin = false) {
  const [fromProfiles, fromUsersPublic, fromRpc, fromPlaylists, fromLikes] = await Promise.all([
